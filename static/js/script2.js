...
const articulaciones = [
    { label: 'Codo derecho', color: 'rgb(0, 255, 0)' },
    { label: 'Hombro derecho', color: 'rgb(255, 0, 0)' },
    { label: 'Codo izquierdo', color: 'rgb(54, 162, 235)' },
    { label: 'Hombro izquierdo', color: 'rgb(255, 206, 86)' },
    { label: 'Muñeca derecha', color: 'rgb(255, 0, 255)' },
    { label: 'Muñeca izquierda', color: 'rgb(255, 75, 8)' },
    { label: 'Cadera Derecha', color: 'rgb(255, 159, 64)' },
    { label: 'Rodilla Derecha', color: 'rgb(153, 102, 255)' },
    { label: 'Tobillo Derecho', color: 'rgb(255, 99, 132)' },
    { label: 'Cadera Izquierda', color: 'rgb(75, 192, 192)' },
    { label: 'Rodilla Izquierda', color: 'rgb(54, 162, 235)' },
    { label: 'Tobillo Izquierdo', color: 'rgb(255, 205, 86)' }
];
...
function onResults(results) {
    ...
    if (results.segmentationMask) {
        ...
        // Obtén los últimos datos de ángulos
        ...
        if (latestData && results.poseLandmarks) {
            ...
            // Dibuja tri/ángulos para el brazo izquierdo
            if (latestData.part === 'leftArm') { // Shoulder angle
                if (jointVisibility.shoulderLeft) {
                    ...
                    drawAngleTriangle(leftShoulder, leftHip, leftElbow, 180 - parseFloat(latestData.angleShoulderLeft), 'rgba(255, 255, 0, 0.5)');
                }
            }

            if (latestData.part === 'leftArm' && results.leftHandLandmarks) { // Elbow angle
                if (jointVisibility.elbowLeft) {
                    const leftShoulder = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_SHOULDER];
                    const leftElbow = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_ELBOW];
                    const leftWrist = results.leftHandLandmarks[0]; //results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_WRIST];

                    drawAngleTriangle(leftElbow, leftShoulder, leftWrist, parseFloat(latestData.angleElbowLeft), 'rgba(0, 0, 255, 0.5)');
                }
            }


            if (latestData.part === 'leftArm' && results.leftHandLandmarks) { // Wrist angle
                if (jointVisibility.wristLeft) {
                    ...
                    drawAngleTriangle(wristLeft, elbowLeft, metacarpalLeft, parseFloat(latestData.angleWristLeft), 'rgb(255, 75, 8, 0.5)');
                }
            }

            if (latestData.part === 'leftLeg') {
                if (jointVisibility.hipLeft) {
                    const leftShoulder = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_SHOULDER];
                    const leftHip = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_HIP];
                    const leftKnee = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_KNEE];

                    drawAngleTriangle(leftHip, leftShoulder, leftKnee, 180 - parseFloat(latestData.angleHipLeft), 'rgba(255, 255, 0, 0.5)');
                }
                if (jointVisibility.kneeLeft) {
                    const leftHip = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_HIP];
                    const leftKnee = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_KNEE];
                    const leftAnkle = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_ANKLE];

                    drawAngleTriangle(leftKnee, leftHip, leftAnkle, parseFloat(latestData.angleKneeLeft), 'rgba(0, 0, 255, 0.5)');
                }
                if (jointVisibility.ankleLeft) {
                    const leftKnee = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_KNEE];
                    const leftAnkle = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_ANKLE];
                    const leftFoot = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.LEFT_FOOT_INDEX];

                    drawAngleTriangle(leftAnkle, leftKnee, leftFoot, parseFloat(latestData.angleAnkleLeft), 'rgba(255, 75, 8, 0.5)');
                }
            }


            //Triángulos para el brazo derecho
            if (latestData.part === 'rightArm' ) {
                if (jointVisibility.shoulderRight) {
                    ...
                    drawAngleTriangle(rightShoulder, rightHip, rightElbow, 180 - parseFloat(latestData.angleShoulderRight), 'rgba(255, 0, 0, 0.5)');
                }
            }


             if (latestData.part === 'rightArm' && results.rightHandLandmarks) {
                 if (jointVisibility.elbowRight) {
                     const rightShoulder = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_SHOULDER];
                     const rightElbow = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_ELBOW];
                     const rightWrist = results.rightHandLandmarks[0]; //results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_WRIST];

                     drawAngleTriangle(rightElbow, rightShoulder, rightWrist, parseFloat(latestData.angleElbowRight), 'rgba(0, 255, 0, 0.5)');
                 }
             }


            if (latestData.part === 'rightArm' && results.rightHandLandmarks) {
                if (jointVisibility.wristRight) {
                    ...
                    drawAngleTriangle(wristRight, elbowRight, metacarpalRight, parseFloat(latestData.angleWristRight), 'rgba(255, 0, 255, 0.5)');
                }
            }

            if (latestData.part === 'rightLeg') {
                if (jointVisibility.hipRight) {
                    const rightShoulder = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_SHOULDER];
                    const rightHip = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_HIP];
                    const rightKnee = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_KNEE];

                    drawAngleTriangle(rightHip, rightShoulder, rightKnee, 180 - parseFloat(latestData.angleHipRight), 'rgba(255, 0, 0, 0.5)');
                }
                if (jointVisibility.kneeRight) {
                    const rightHip = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_HIP];
                    const rightKnee = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_KNEE];
                    const rightAnkle = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_ANKLE];

                    drawAngleTriangle(rightKnee, rightHip, rightAnkle, parseFloat(latestData.angleKneeRight), 'rgba(0, 255, 0, 0.5)');
                }
                if (jointVisibility.ankleRight) {
                    const rightKnee = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_KNEE];
                    const rightAnkle = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_ANKLE];
                    const rightFoot = results.poseLandmarks[mpHolistic.POSE_LANDMARKS.RIGHT_FOOT_INDEX];

                    drawAngleTriangle(rightAnkle, rightKnee, rightFoot, parseFloat(latestData.angleAnkleRight), 'rgba(255, 0, 255, 0.5)');
                }
            }
        }
